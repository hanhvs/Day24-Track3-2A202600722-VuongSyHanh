...
'''
    ...
    # Fallback heuristic (no API key hoặc khi call thất bại): chọn theo độ dài hợp lý.
    def _heuristic() -> dict:
        la, lb = len(answer_a.strip()), len(answer_b.strip())
        if la == 0 and lb == 0:
            return {"winner": "tie", "reasoning": "Cả hai answer đều rỗng.",
                    "scores": {"A": 0.0, "B": 0.0}}
        if abs(la - lb) <= 5:
            return {"winner": "tie", "reasoning": "Hai answer tương đương nhau.",
                    "scores": {"A": 0.5, "B": 0.5}}
        winner = "A" if la > lb else "B"
        return {"winner": winner,
                "reasoning": "Heuristic fallback (không có LLM): chọn answer đầy đủ hơn.",
                "scores": {"A": round(la / (la + lb), 3), "B": round(lb / (la + lb), 3)}}
    ...
    try:
        ...
        print(f"  ⚠️  pairwise_judge LLM call failed: {e} — dùng heuristic fallback.")
        return _heuristic()
...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    p2_scores = pass2_raw.get("scores", {})
    return JudgeResult(
        question=question, answer_a=answer_a, answer_b=answer_b,
    ...
    if total == 0:
        return {"total_judged": 0, "position_bias_rate": 0.0, "position_bias_count": 0,
                ...
                "verbosity_details": {"a_wins_a_longer": 0, "b_wins_b_longer": 0,
                                      "total_decisive": 0},
                "interpretation": "Không có kết quả nào để phân tích."}
...
def save_phase_b_report(judge_results: list[JudgeResult], bias: dict,
                        kappa: float, kappa_detail: list[dict],
    ...
    report = {
        ...
        "kappa_interpretation": kappa_interp(kappa),
...
if __name__ == "__main__":
    ...
    # Map question_id → ground_truth từ test_set 50q
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        test_set = json.load(f)
    gt_by_id = {item["id"]: item["ground_truth"] for item in test_set}

    # Chạy judge tuyệt đối trên 10 câu human-labeled → judge_labels
    print("Running judge on 10 human-labeled questions...")
    judge_labels = []
    kappa_detail = []
    for item in human_data:
        gt = gt_by_id.get(item["question_id"], "")
        jl = quality_label(item["question"], item["model_answer"], gt)
        judge_labels.append(jl)
        kappa_detail.append({
            "question_id": item["question_id"],
            "human_label": item["human_label"],
            "judge_label": jl,
            "agree": item["human_label"] == jl,
        })

    kappa = cohen_kappa(judge_labels, human_labels)
    print(f"Cohen's κ: {kappa:.3f}")

    # --- Pairwise trên vài cặp (model_answer vs ground_truth) để bias report ---
    judge_results = [result]
    for item in human_data[:5]:
        gt = gt_by_id.get(item["question_id"], "")
        if gt:
            judge_results.append(
                swap_and_average(item["question"], item["model_answer"], gt))

    # --- Bias report ---
    bias = bias_report(judge_results)
    print(f"\nBias report: {bias}")

    save_phase_b_report(judge_results, bias, kappa, kappa_detail)
